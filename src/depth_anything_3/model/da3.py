...
class DepthAnything3Net(nn.Module):
    ...
    def forward(
        ...
        if extrinsics is not None:
            with torch.autocast(device_type=x.device.type, enabled=False):
                cam_token = self.cam_enc(extrinsics, intrinsics, x.shape[-2:])  # type: ignore
        else:
            cam_token = None
    ...
    def _process_mono_sky_estimation(self, output: Any) -> Any:
    ...
    def _process_ray_pose_estimation(self, output: Any, height: int, width: int) -> Any:
...
class NestedDepthAnything3Net(nn.Module):
    ...
    def _apply_metric_scaling(self, output: Any, metric_output: Any) -> Any:
    ...
    def _apply_depth_alignment(self, output: Any, metric_output: Any) -> Any:
