...
class DualDPT(nn.Module):
    ...
    def _forward_impl(
        ...
        main_logits = scratch.output_conv2(fused_main)
        ...
        last_aux_logits = scratch.output_conv2_aux[-1](last_aux)
    ...
    def _fuse(
        ...
        l1_rn = scratch.layer1_rn(l1)
        l2_rn = scratch.layer2_rn(l2)
        l3_rn = scratch.layer3_rn(l3)
        l4_rn = scratch.layer4_rn(l4)

        # level 4 -> 3
        out = scratch.refinenet4(l4_rn, size=l3_rn.shape[2:])
        aux_out = scratch.refinenet4_aux(l4_rn, size=l3_rn.shape[2:])
        aux_list: list[torch.Tensor] = []
        if self.aux_levels >= 4:
            aux_list.append(aux_out)

        # level 3 -> 2
        out = scratch.refinenet3(out, l3_rn, size=l2_rn.shape[2:])
        aux_out = scratch.refinenet3_aux(aux_out, l3_rn, size=l2_rn.shape[2:])
        if self.aux_levels >= 3:
            aux_list.append(aux_out)

        # level 2 -> 1
        out = scratch.refinenet2(out, l2_rn, size=l1_rn.shape[2:])
        aux_out = scratch.refinenet2_aux(aux_out, l2_rn, size=l1_rn.shape[2:])
        if self.aux_levels >= 2:
            aux_list.append(aux_out)

        # level 1 (final)
        out = scratch.refinenet1(out, l1_rn)
        aux_out = scratch.refinenet1_aux(aux_out, l1_rn)
        aux_list.append(aux_out)

        out = scratch.output_conv1(out)
        aux_list = [scratch.output_conv1_aux[i](aux) for i, aux in enumerate(aux_list)]
