...
def export_to_npz(
    ...
    save_dict = {
        "image": image,
        "depth": prediction.depth.copy(),  # Store depth with full precision (no rounding) for reproducibility
    }

    if prediction.conf is not None:
        save_dict["conf"] = prediction.conf.astype(
            np.float32
        )  # Store confidence with full precision (no rounding) for reproducibility
